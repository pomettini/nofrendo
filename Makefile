...
bench-kirby-irqpair:
	cmake -B build/device -DTOOLCHAIN=armgcc -DCMAKE_TOOLCHAIN_FILE=$(TOOLCHAIN) $(FAST_FLAGS) -DNES_IRQ_MAPPER_BATCH_IRQ_SCOPE=ON -DPPU_BG_PAIR_FAST=ON -DPD_PLAYBENCH_KIRBY=ON
	cmake --build build/device
	cmake -B build/sim $(FAST_FLAGS) -DNES_IRQ_MAPPER_BATCH_IRQ_SCOPE=ON -DPPU_BG_PAIR_FAST=ON -DPD_PLAYBENCH_KIRBY=ON
	cmake --build build/sim
...
bench-kirby-profile:
	cmake -B build/device -DTOOLCHAIN=armgcc -DCMAKE_TOOLCHAIN_FILE=$(TOOLCHAIN) $(FAST_FLAGS) -DNES_IRQ_MAPPER_BATCH_IRQ_SCOPE=ON -DPPU_BG_PAIR_FAST=ON -DPD_PLAYBENCH_KIRBY=ON -DDIAG=ON -DDIAG_CPU_EXEC_TIMING=ON
	cmake --build build/device
	cmake -B build/sim $(FAST_FLAGS) -DNES_IRQ_MAPPER_BATCH_IRQ_SCOPE=ON -DPPU_BG_PAIR_FAST=ON -DPD_PLAYBENCH_KIRBY=ON -DDIAG=ON -DDIAG_CPU_EXEC_TIMING=ON
	cmake --build build/sim
